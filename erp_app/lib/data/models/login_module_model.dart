...
class GuardedNavigationBuilder {
  ...
  // تنظیم route
  GuardedNavigationBuilder addRoute(String path,
  ...
  GuardedNavigationBuilder setTransitionsBuilder(
      Widget Function(BuildContext, Animation<double>, Animation<
          double>, Widget)
      builder,) {
  ...
  NavigationBuilder build() {
    if (!isReady) {
      throw StateError(
        'Cannot build NavigationBuilder. Missing required parameters: ${missingParams
            .join(', ')}',
      );
    }
    ...
    final Map<String, Widget Function(RouteData)> finalRoutes = {};
