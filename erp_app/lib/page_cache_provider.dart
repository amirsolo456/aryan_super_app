// lib/core/cache/page_cache_provider.dart
import 'dart:async';
...
class PageCacheProvider extends ChangeNotifier {
  ...
  Map<NavButtonTabBarMode, Widget> _tabPage = {};
  ...
  // تغییر تب و تمیز کردن کش
  // void changeTab(NavButtonTabBarMode newTab) {
  //   // اگر در حال خروج هستیم، اجازه تغییر تب ندهیم
  //   if (_isProcessingSignOut) return;
  //
  //   _selectedTab = newTab;
  //   _isListRoute = _defValue;
  //   _isErrorsState = false;
  //   _isFormRoute = _defValue;
  //   _cacheManager.cleanCacheExcept(newTab.value);
  //   notifyListeners();
  // }
  ...
  Future<void> signOut({bool force = false}) async {
    ...
    // مرحله ۱: کنسل کردن تمام عملیات در حال اجرا
    await _cancelAllPendingOperations();

    // مرحله ۲: متوقف کردن تمام تایمرها
    _cancelAllTimers();

    // مرحله ۳: لغو تمام subscriptionها
    _cancelAllSubscriptions();

    // مرحله ۴: پاک کردن کش‌ها
    _clearAllCaches();

    // مرحله ۵: پاک کردن استیت‌ها
    _resetAllStates();

    // اطلاع‌رسانی تکمیل خروج
    /*    GetIt.instance<AppNotifier>().notify(
      type: NotificationType.sessionExpired,
      message: 'Sign out completed successfully',
      source: 'PageCacheProvider',
    );*/

    // اگر force نباشد، می‌توانیم منتظر بمانیم تا UI به‌روزرسانی شود
    if (!force) {
      await Future.delayed(const Duration(milliseconds: 500));
    }

    // علامت‌گذاری خروج کامل
    _isSignOut = true;
    notifyListeners();
  }
  ...
  Future<void> _cancelAllPendingOperations() async {
    ...
    GetIt.instance<AppNotifier>().notifyInfo(
      'Cancelling ${_pendingOperations.length} pending operations',
    );
*/

    final List<Future<void>> cancellationFutures = [];
  ...
  void _cancelAllTimers() {
    ...
    /*
    GetIt.instance<AppNotifier>().notifyInfo(
      'Cancelling ${_activeTimers.length} active timers',
    );
*/
